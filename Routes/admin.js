  ...
  try {
    ...
    if (admin) {
      const token = jwt.sign(
        { _id: admin._id, role: admin.Role, name: admin.Name },
        process.env.SECRET
      );
...
Router.put("/manager/:id", async (req, res) => {
  try {
    const updatedManager = await AdminModel.updateOne(
      { _id: req.params.id },
      {
        $set: {
          Name: req.body.Name,
  ...
  fileStream
    ...
    .on("finish", async () => {
      ...
      try {
        ...
        return res
          .status(500)
          .json({ message: "Error saving product to database." });
      }
    });
});
...
Router.put("/product/variation/:id", async (req, res) => {
  try {
    ...
    const oldVariationIndex = prod.Variations.findIndex(
      (variation) => variation.name === req.body.Variation.name
    );
    ...
    if (oldVariationIndex !== -1) {
      ...
      return res
        .status(200)
        .json({ product: updatedProduct, message: "Variation updated" });
    } else {
